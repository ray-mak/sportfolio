...
const createMmaMLBet = asyncHandler(async (req, res) => {
    const {user, betType, event, fight, pick, odds, notes} = req.body

    if(!user || !betType || !event || !fight || !pick || !odds) {
        return res.status(400).json({message: 'All fields are required'})
    }
    
    const mlBet = await mmaMLBet.create({user, betType, event, fight, pick, odds, notes})
    if (mlBet) {

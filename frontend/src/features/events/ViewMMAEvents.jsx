...
const ViewMMAEvents = () => {
    ...
    if (isSuccess) {
        const upcomingEvents = data.upcomingEvents.map(object => {
            return (
                <tr key={object._id}>
                    <td>{formatDate(object.eventDate)}</td>
                    <td onClick={() => navigate(`/mmaevents/${object._id}`)} className="underline font-medium cursor-pointer">{object.eventName}</td>
                </tr>
            )
        })
        ...
        const pastEvents = data.pastEvents.map(object => {
            return (
                <tr key={object._id}>
                    <td>{formatDate(object.eventDate)}</td>
                    <td className="underline">{object.eventName}</td>
                </tr>
            )
        })
